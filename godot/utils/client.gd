...
func _init(url: String, on_socket_event = null, on_socket_ready = on_socket_ready, on_socket_connect = on_socket_connect):
	url = _preprocess_url(url)
	_url = "%s?EIO=4&transport=websocket" % url
	

	# this signal is emitted when the socket is ready to connect
	self.on_engine_connected.connect(on_socket_ready)

	# this signal is emitted when socketio server is connected
	self.on_connect.connect(on_socket_connect)

	# this signal is emitted when socketio server sends a message
	self.on_event.connect(on_socket_event)
